...
- (void)viewDidLoad {
    ...
    skip.frame = Frm(30, 10, 50, 50);
    skip.layer.borderWidth = 1;
    skip.layer.cornerRadius = 25;
    skip.layer.borderColor = [@"#FC484D" hexColor].CGColor;
    skip.backgroundColor = [UIColor whiteColor];
    [skip setTitleColor:[@"#FC484D" hexColor] forState:UIControlStateNormal];
    skip.titleLabel.font = Bold(20);
    skip.enabled = NO;
    [skip setTitle:Lang(@"index_skip") selected:Lang(@"index_skip")];
    [skip addTarget:self action:@selector(skipThis:)];
    [btn addSubview:skip];

    finish = [UIButton buttonWithType:UIButtonTypeSystem];
    ...
    finish.frame = Frm(skip.endX + 15, 10, 200, 50);
